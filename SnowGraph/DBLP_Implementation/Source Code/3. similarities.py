...
InputXlsxFile = "metis.xlsx"   #input file containing data to be processed.
...
for key, value in sorted(author_dict.iteritems(), key=lambda (k,v): (v,k)):
    f.write("A%s\t%s\n" % (value, key))      #Print the entire author key mapping onto the AuthorKey file.
    BPGAuthorFile += "\n" + "A" + str(value)
    h.write(key)
    h.write("\n")
...
for auth_counter in range(author_count):
    for year_counter in range(len(conf_list)):
        ...
        if BPGmax_value < BPGvalue:
            BPGmax_value = BPGvalue
for auth_counter in range(author_count):
    for year_counter in range(len(conf_list)):
        BPGvalue = BPGraph_dict[auth_counter][year_counter]
        if (BPGvalue >0):
            ...
            BPGdata += ('\nA{}\tC{}\tedge\t{}'.format((auth_counter+1), (year_counter + 2001), BPGvalue/BPGmax_value))
