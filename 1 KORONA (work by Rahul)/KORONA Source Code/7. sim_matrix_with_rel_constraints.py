...
#../util/compute_distance_matrix_ncut.py  0.3684 0.1832 ../semEP-constraint/datasets/yamanishi/nr/nr_matrix_drugs.txt ../semEP-constraint/datasets/yamanishi/nr/nr_drugs.txt ../semEP-constraint/datasets/yamanishi/nr/nr_matrix_targets.txt ../semEP-constraint/datasets/yamanishi/nr/nr_targets.txt ../semEP-constraint/datasets/yamanishi/nr/nr_drug-target_graph.txt ncut_yamanishi_symmetric_matrix/nr_p80.txt

#./sim_matrix_with_rel_constraints.py <threshold_1> <threshold_2> <matrix_1> <entities_1> <matrix_2> <entities_2> <graph> <output_file>
import sys, os
...
def print_matrix(filename, simM, nele):
    ...
    for i in range(nele):
        for j in range(nele-1):
            #fd.write("{0:.2f},".format(simM[i][j]))
            fd.write(str(simM[i][j])+",")
            #fd.write("1.0,")
        fd.write(str(simM[i][nele-1])+"\n")
        #fd.write("1.0\n")
        #fd.write("{0:.2f}\n".format(simM[i][nele-1]))
...
if __name__ == "__main__":
    ...
    print("Done! output filename: "+sys.argv[8])
